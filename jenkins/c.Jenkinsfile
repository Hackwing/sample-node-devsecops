pipeline {
    ...
    environment {
        DOCKER_IMAGE = 'myapp-c:$BUILD_NUMBER'
            ...
            steps {
                dir('c') {
                    withSonarQubeEnv('SonarQube') {
                        withEnv(["SONAR_TOKEN=sqa_cdef3df12dadf103a42d232db587c55c7a30cb55"]) {
                            sh 'npx sonar-scanner -Dsonar.host.url=http://sonarqube:9000 -Dsonar.login=$SONAR_TOKEN'
                        }
                    }
                }
            }
